...
type RequestSettings = {
  ...
  setRequestType: (val: "userLogin" | "userRegister" | "atRegister")=> void;
}
type LoginFormType = {
  email: string;
  password: string;
...
export const LoginForm = ({setRequestSuccess, setRequestError, setRequestType} :RequestSettings) =>{
  const navigate = useNavigate()
    ...
    <Box 
      ...
      <Typography variant='h5' >Bienvenid@, ingrese por favor</Typography>
      <Stack spacing={0}>
        ...
        <Button className="submit-button" type='submit' variant="contained" color='primary' >Ingresar</Button>
        <Button variant="outlined" color='primary' onClick={(e: React.FormEvent)=>{e.preventDefault(); alert("Aun no hago nada")}}>Olvide mi contraseña</Button>
        <Button variant="outlined" color='primary' onClick={(e: React.FormEvent)=>{e.preventDefault(); setRequestType("userRegister")}}>No tengo una cuenta</Button>
...
export const RegisterForm = ({setRequestSuccess, setRequestError} :RequestSettings) =>{
      ...
      const context = useContext(UserContext);
        ...
        const {setUser} = context
      const [registerType,setRegisterType] = useState<"userRegister" | "atRegister">("userRegister")
      const {
        control,
        register,
        handleSubmit,
        ...
        useEffect(() => {
            ...
            if (isSubmitSuccessful) {
                setRequestSuccess(registerType);
                console.log("Envio al backend correcto");
            }
            if(errors && errors.title?.message) {
            console.log("error de contraseñas")
        ...
        await new Promise(res => setTimeout(res, 3000));
          ...
          // Archivos (si existen)
          if (registerType === "atRegister" && data.profileImage?.[0]) {
            formData.append("profileImage", data.profileImage[0]);
          }
          else if (registerType === "atRegister"){
            setError("title",{message: "El obligatoria una foto de perfil"})
            return
          }
          if (registerType === "atRegister" && data.titleImage?.[0]) {
...
return (
 ...
 sx={{maxWidth: 400, mx: "auto"}} >
      <Typography variant='h5' >Registrese</Typography>
      <Stack spacing={0}>
        ...
        <div className="login-inputs inputs-container">
            ...
            <div id="checkbox-container"><label>Deseo registrarme como profesional</label><input type="checkbox" onClick={()=>{if (registerType === "userRegister") setRegisterType("atRegister"); else setRegisterType("userRegister")}} /></div>
        </div>
        {registerType === "atRegister" 
          ? <div className="terapeuts-inputs inputs-container">  
              ...
              <Controller
                  control={control}
                  name="title"
                  rules={{ required: "No ha indicado su título" }}
                  render={({ field }) => (
                    <Select
                        {...field}
                        labelId="title-label"
        ...
        <Button className="submit-button" type='submit' variant="contained" color='primary' >Registrarse</Button>
      </Stack>
    </Box>
    </>
  );
